...
class PauseSubState extends MusicBeatSubstate
{
	...
	public function new(x:Float, y:Float)
	{
		...
		var currentSong = PlayState.SONG.song;
		if (currentSong == 'Obituary' && ProgressionHandler.isRodent) currentSong = 'Free-4-Me';

		var levelInfo:FlxText = new FlxText(20, 15, 0, "", 32);
		levelInfo.text += currentSong;
			...
			switch (daSelected)
			{
				...
				case 'Options':
					...
					if (ProgressionHandler.isRodent) {
						...
						MusicBeatState.switchState(new meta.states.desktop.DesktopOptionsState());
						if(ClientPrefs.data.pauseMusic != 'None')
						{
							FlxTween.tween(FlxG.sound.music, {volume: 1}, 0.8);
							FlxG.sound.music.time = pauseMusic.time;
